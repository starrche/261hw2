...
/* Resizes the underlying array to be the size cap
	...
	pre:	v is not null. newCap should be greater than old cap
	post:	v has capacity newCap
*/
void _dynArrSetCapacity(DynArr *v, int newCap)
{
  assert((v!=NULL) && (newCap > v->capacity ));
  /* FIXME: You will write this function */
	/* memorize old data */
  TYPE * data_copy = v->data;
  int size_copy = v->size;
  /* allocate new memory */
  initDynArr (v, newCap);
  for (int i = 0; i < size_copy; i++)
    v->data[i] = data_copy[i];
  v->size = size_copy;
  /* free memory */
  free(data_copy);
...
{
	/* FIXME: You will write this function */
  /* the top of stack is the last element of dynArray */
  addDynArr(v,val);
...
void popDynArr(DynArr *v)
{
  ...
  assert(v->size > 0);
  v->size--;
  /* all pop does is decrement size?? */
...
*/
/* only returns index of 1st element found */
